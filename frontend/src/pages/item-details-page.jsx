...
import { getItemTransactions } from "../api/transactions"
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <Button variant="ghost" onClick={() => navigate("/items")} className="hover:bg-gray-100 self-start">
        ...
        {(canUpdateItem || canDeleteItem) && (
          <div className="flex space-x-2">
            {canUpdateItem && (
              <Button onClick={() => navigate(`/items/${id}/edit`)} variant="outline">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="font-medium">${Number.parseFloat(item.sale_price).toFixed(2)}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <div className="flex items-center">
                        <span className="font-medium mr-2">{item.current_stock}</span>
                        <Badge
                          className={
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <div className="w-32 bg-gray-200 rounded-full h-2.5">
                          <div
                            ...
                            style={{
                              width: `${Math.min(100, (item.current_stock / (item.reorder_point * 2 || 10)) * 100)}%`,
                            }}
                          ></div>
                        </div>
                      </div>
                      {item.current_stock <= (item.reorder_point || 5) && item.current_stock > 0 && (
                        <p className="text-sm text-yellow-600">
                          This item is below the reorder point. Consider restocking soon.
                        </p>
                      )}
                      {item.current_stock <= 0 && (
                        <p className="text-sm text-red-600">This item is out of stock. Restock immediately.</p>
                      )}
                    </div>
                  </div>
                </div>
          ...
          <TabsContent value="transactions" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>View all transactions related to this item</CardDescription>
              </CardHeader>
              <CardContent>
                      ...
                      <TableBody>
                        {transactions.map((transaction) => (
                          ...
                          >
                              ...
                              <Badge
                                className={
                                  transaction.transaction_type === "sale"
                                    ...
                                    : transaction.transaction_type === "initial_stock"
                                      ? "bg-green-100 text-green-800"
                            ...
                            <TableCell>{transaction.recorded_by?.username || "System"}</TableCell>
                            <TableCell>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          ...
          <div className="py-4">
            <p>
              You are about to delete <strong>{item.item_name}</strong> (SKU: {item.SKU})
            </p>
            {item.current_stock > 0 && (
              <p className="mt-2 text-amber-600">
                Warning: This item has {item.current_stock} units in stock. Deleting it will remove all inventory
                records.
              </p>
            )}
          </div>
